/**
 * Generate bedroom-specific property pages for major cities only
 * Focus: Nairobi, Mombasa, Kisumu, Eldoret neighborhoods & estates
 */

const fs = require('fs');
const path = require('path');

// Bedroom configurations to generate
const BEDROOM_CONFIGS = [
  { beds: 2, slug: '2-bedroom-houses', name: '2 Bedroom House', plural: '2 Bedroom Houses' },
  { beds: 3, slug: '3-bedroom-houses', name: '3 Bedroom House', plural: '3 Bedroom Houses' },
  { beds: 4, slug: '4-bedroom-houses', name: '4 Bedroom House', plural: '4 Bedroom Houses' },
  { beds: 5, slug: '5-bedroom-houses', name: '5 Bedroom House', plural: '5 Bedroom Houses' },
];

// Transaction types
const TRANSACTIONS = ['sale', 'rent'];

// Target cities with high search volume
const TARGET_CITIES = ['Nairobi', 'Mombasa', 'Kisumu', 'Eldoret'];

// Template for bedroom-specific pages
const PAGE_TEMPLATE = `import { Metadata } from 'next';
...
const DB_QUERY = '%house%|%bungalow%|%maisonette%{{VILLA_QUERY}}';
const PROPERTY_LABEL = '{{PROPERTY_LABEL}}';
const BEDROOM_COUNT = {{BEDROOM_COUNT}};
...
// Generate all bedroom pages
let totalGenerated = 0;

BEDROOM_CONFIGS.forEach(config => {
  TRANSACTIONS.forEach(transaction => {
    const folderName = `${config.slug}-for-${transaction}`;
    ...
    const locationFolderPath = path.join(folderPath, '[location]');
    ...
    let pageContent = PAGE_TEMPLATE
      .replace(/{{PROPERTY_TYPE}}/g, config.slug)
      .replace(/{{TRANSACTION_TYPE}}/g, transaction)
      .replace(/{{PROPERTY_LABEL}}/g, config.plural)
      .replace(/{{BEDROOM_COUNT}}/g, config.beds.toString())
      .replace(/{{PRICE_TYPE}}/g, transaction === 'sale' ? 'Sale' : 'Rent')
      .replace(/{{VILLA_QUERY}}/g, config.beds >= 4 ? '|%villa%' : '');

    // Write file
    const filePath = path.join(locationFolderPath, 'page.tsx');
    fs.writeFileSync(filePath, pageContent);

    totalGenerated++;
    console.log(`✅ Created ${folderName}/[location]/page.tsx`);
  });
});

console.log(`\n🎉 Generated ${totalGenerated} bedroom-specific property pages!`);
console.log(`📍 Targeting: Nairobi, Mombasa, Kisumu, Eldoret neighborhoods & estates only`);
console.log(`🛏️  Bedroom types: 2BR, 3BR, 4BR, 5BR (Sale & Rent)`);
