...
const PROPERTY_TYPE = 'land';
const TRANSACTION_TYPE = 'sale';
const DB_QUERY = '%land%|%plot%';
const PROPERTY_LABEL = 'Land Plots';
...
async function getSmartAlternatives(
  ...
  if (cityFilter || location.type !== 'county') {
    let query3 = supabase
      ...
      .eq('is_approved', true)
      .eq('price_type', 'sale')
      .not('property_type', 'ilike', '%land%') // Different property types
      .ilike('county', `%${countyName}%`);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
          ...
          <>
            ...
            {smartAlternatives.sameCityDifferentBedrooms.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.city
                    ? `Other Land for Sale in ${searchParams.city}`
                    : `Other Land for Sale in ${location.name}`}
                </h2>
                <p className="text-gray-600 mb-6">
            ...
            {smartAlternatives.sameBedroomsDifferentCity.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.bedrooms
                    ? `${searchParams.bedrooms}-Bedroom Land in ${location.county}`
                    : `Land for Sale in ${location.county}`}
                </h2>
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
            ...
            {smartAlternatives.differentPropertyTypes.length > 0 && (
              <div className="mb-12">
                ...
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
                    ? `Check out these ${searchParams.bedrooms}-bedroom properties (Houses, Apartments, Commercial Properties, etc.) ${searchParams?.city ? `in ${searchParams.city}` : `in ${location.name}`}:`
        ...
        {properties.length > 0 && properties.length < 3 && alternativeProperties.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              More Land for Sale in {location.county}
            </h2>
            <p className="text-gray-600 mb-6">
