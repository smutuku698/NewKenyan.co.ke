...
async function getSmartAlternatives(
  ...
  // Query 3: Different property types in same location (for bungalows, show apartments, villas, etc.)
  if (cityFilter || location.type !== 'county') {
    let query3 = supabase
      ...
      .eq('is_approved', true)
      .eq('price_type', 'rent')
      .not('property_type', 'ilike', '%bungalow%')
      .ilike('county', `%${countyName}%`);
          ...
          <>
            ...
            {smartAlternatives.differentPropertyTypes.length > 0 && (
              <div className="mb-12">
                ...
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
                    ? `Check out these ${searchParams.bedrooms}-bedroom properties (Villas, Maisonettes, Apartments, etc.) ${searchParams?.city ? `in ${searchParams.city}` : `in ${location.name}`}:`
