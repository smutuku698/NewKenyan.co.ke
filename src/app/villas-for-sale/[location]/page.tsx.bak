...
async function getAlternativeProperties(location: Location, limit: number = 8): Promise<PropertyListing[]> {
  ...
  // Get villas for sale from same county
  let query = supabase
    .from('property_listings')
    .select('*')
    .eq('is_approved', true)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
          ...
          <>
            <div className="text-center py-12 mb-12 bg-white rounded-lg shadow-sm">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                No {PROPERTY_LABEL.toLowerCase()} for sale found in {location.name}
              </h3>
            ...
            {alternativeProperties.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {PROPERTY_LABEL} for Sale in {location.county}
                </h2>
                <p className="text-gray-600 mb-6">
                  Check out these {PROPERTY_LABEL.toLowerCase()} for sale in nearby areas within {location.county}
                </p>
        ...
        {properties.length > 0 && properties.length < 3 && alternativeProperties.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              More {PROPERTY_LABEL} for Sale in {location.county}
            </h2>
            <p className="text-gray-600 mb-6">
              Explore more {PROPERTY_LABEL.toLowerCase()} for sale in nearby areas
            </p>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
