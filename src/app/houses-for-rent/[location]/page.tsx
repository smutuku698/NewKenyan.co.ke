...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return generateLocationMetadata(location, 'houses', 'rent', stats);
}
...
export default async function HousesForRentPage({ params }: PageProps) {
  ...
  const h1 = generateH1(location, 'houses', 'rent', h1VariationIndex);

  const breadcrumbItems = generateBreadcrumbs(location, 'houses', 'rent');
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600 max-w-3xl">
            {location.description || `Discover ${stats.totalCount} house${stats.totalCount !== 1 ? 's' : ''} for rent in ${location.name}.
            Find your perfect rental home with verified listings, photos, and direct landlord contact.`}
          </p>
        </div>
