...
async function getApartmentsForRent(
  location: Location,
  searchParams?: { [key: string]: string | string[] | undefined }
...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  const properties = await getApartmentsForRent(location, searchParams);
...
export default async function ApartmentsForRentPage({ params, searchParams }: PageProps) {
  const location = await getLocation(params.location);
  ...
  const h1 = generatePropertyH1(location, 'apartments', 'rent', h1VariationIndex);

  const breadcrumbItems = generatePropertyBreadcrumbs(location, 'apartments', 'rent');
  const aboutContent = generateAboutContent(location, 'apartments', 'rent', stats);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
          ...
          <>
            <div className="text-center py-12 mb-12">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                No apartments for rent found {searchParams?.city ? `in ${searchParams.city}, ${location.name}` : `in ${location.name}`}
              ...
              <p className="text-gray-600 mb-6">
                {searchParams?.city || searchParams?.bedrooms
                  ...
                  : `Be the first to list an apartment for rent in ${location.name}!`
                }
              </p>
              <a
                href="/add-listing"
            ...
            {smartAlternatives.sameCityDifferentBedrooms.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.city
                    ? `Other Apartments for Rent in ${searchParams.city}`
                    : `Other Apartments for Rent in ${location.name}`}
                </h2>
                <p className="text-gray-600 mb-6">
              ...
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.bedrooms
                    ...
                    : `Apartments for Rent in ${location.county}`}
                </h2>
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
            ...
            {smartAlternatives.differentPropertyTypes.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.city
                    ? `Other Property Types for Rent in ${searchParams.city}`
                    : `Other Property Types for Rent in ${location.name}`}
                </h2>
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
            ...
            {smartAlternatives.sameCityDifferentBedrooms.length === 0 &&
             ...
             alternativeProperties.length > 0 && (
              <div className="mb-12">
                ...
                <p className="text-gray-600 mb-6">
                  Check out these apartments for rent in {location.county}
                </p>
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {properties.length > 0 && properties.length < 3 && alternativeProperties.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              More Apartments for Rent in {location.county}
            </h2>
            <p className="text-gray-600 mb-6">
              Explore more apartments for rent in nearby areas
            </p>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="bg-white p-8 rounded-lg shadow-sm mb-8">
          <h2 className="text-2xl font-bold mb-4">Rent an Apartment in {location.name}</h2>
          <div className="prose max-w-none text-gray-600">
            <p className="mb-4">
              {location.description || `${location.name} offers a variety of apartments for rent in ${location.county}, Kenya,
              from affordable bedsitters to luxury penthouses.`}
            </p>

            {location.type === 'county' && (
              <>
                <h3 className="text-lg font-semibold mt-6 mb-3">Why Rent an Apartment in {location.name} County?</h3>
                <ul className="list-disc pl-6 space-y-2">
                  <li>Modern apartment complexes with great amenities</li>
                  <li>Flexible lease terms from short to long-term</li>
                  <li>Secure gated communities with 24/7 security</li>
                  <li>Lower utility costs and maintenance-free living</li>
                </ul>
              </>
            )}
            ...
            {location.type === 'neighborhood' && (
              <>
                ...
                <ul className="list-disc pl-6 space-y-2">
                  <li>Convenient location with easy access to amenities</li>
                  <li>Well-maintained buildings with professional management</li>
                  <li>Great for young professionals and families</li>
                  <li>Access to public transport and major roads</li>
                </ul>
              </>
            )}
            ...
            {location.type === 'estate' && stats.popularAmenities.length > 0 && (
              <>
                <h3 className="text-lg font-semibold mt-6 mb-3">Amenities in {location.name}</h3>
