...
async function getSmartAlternatives(
  ...
  // Query 3: Different property types in same location (for townhouses, show apartments, bungalows, maisonettes, etc.)
  if (cityFilter || location.type !== 'county') {
    let query3 = supabase
      ...
      .eq('is_approved', true)
      .eq('price_type', 'rent')
      .not('property_type', 'ilike', '%townhouse%') // Different property types
      .ilike('county', `%${countyName}%`);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
          ...
          <>
            <div className="text-center py-12 mb-12">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                No townhouses for rent found {searchParams?.city ? `in ${searchParams.city}, ${location.name}` : `in ${location.name}`}
              ...
              <p className="text-gray-600 mb-6">
                {searchParams?.city || searchParams?.bedrooms
                  ? `Try adjusting your filters or be the first to list a townhouse ${searchParams?.city ? `in ${searchParams.city}` : `in ${location.name}`}!`
                  : `Be the first to list a townhouse for rent in ${location.name}!`
                }
              </p>
              <a
                href="/add-listing"
            ...
            {smartAlternatives.sameCityDifferentBedrooms.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.city
                    ? `Other Townhouses for Rent in ${searchParams.city}`
                    : `Other Townhouses for Rent in ${location.name}`}
                </h2>
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
                    ? `No ${searchParams.bedrooms}-bedroom townhouses available. Check out these other options ${searchParams?.city ? `in ${searchParams.city}` : `in ${location.name}`}:`
                    : `Available townhouses ${searchParams?.city ? `in ${searchParams.city}` : `in ${location.name}`}:`}
                </p>
            ...
            {smartAlternatives.sameBedroomsDifferentCity.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.bedrooms
                    ? `${searchParams.bedrooms}-Bedroom Townhouses in ${location.county}`
                    : `Townhouses for Rent in ${location.county}`}
                </h2>
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
                    ? `Check out these ${searchParams.bedrooms}-bedroom townhouses in other areas of ${location.county}:`
                    : `Available townhouses in nearby areas of ${location.county}:`}
                </p>
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            {smartAlternatives.sameCityDifferentBedrooms.length === 0 &&
             ...
             alternativeProperties.length > 0 && (
              <div className="mb-12">
                ...
                <p className="text-gray-600 mb-6">
                  Check out these townhouses for rent in {location.county}
                </p>
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {properties.length > 0 && properties.length < 3 && alternativeProperties.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              More Townhouses for Rent in {location.county}
            </h2>
            <p className="text-gray-600 mb-6">
              Explore more townhouses for rent in nearby areas
            </p>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
