...
const PROPERTY_TYPE = 'commercial-properties';
const TRANSACTION_TYPE = 'rent';
const DB_QUERY = '%commercial%';
const PROPERTY_LABEL = 'Commercial Properties';
...
async function getSmartAlternatives(
  ...
  // Query 3: Different property types in same location (for commercial, show offices, shops, warehouses, etc.)
  if (cityFilter || location.type !== 'county') {
    let query3 = supabase
      ...
      .eq('is_approved', true)
      .eq('price_type', 'rent')
      .not('property_type', 'ilike', '%commercial%') // Different property types
      .ilike('county', `%${countyName}%`);
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {stats.totalCount > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border text-center">
              <div className="text-lg font-bold text-green-600">12.0%</div>
          ...
          <>
            <div className="text-center py-12 mb-12">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                No {PROPERTY_LABEL.toLowerCase()} for rent found {searchParams?.city ? `in ${searchParams.city}, ${location.name}` : `in ${location.name}`}
              ...
              <p className="text-gray-600 mb-6">
                {searchParams?.city || searchParams?.bedrooms
                  ...
                  : `Be the first to list a property for rent in ${location.name}!`
                }
              </p>
              <a
                href="/add-listing"
            ...
            {smartAlternatives.sameCityDifferentBedrooms.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.city
                    ? `Other ${PROPERTY_LABEL} for Rent in ${searchParams.city}`
                    : `Other ${PROPERTY_LABEL} for Rent in ${location.name}`}
                </h2>
                <p className="text-gray-600 mb-6">
            ...
            {smartAlternatives.sameBedroomsDifferentCity.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchParams?.bedrooms
                    ? `${searchParams.bedrooms}-Bedroom ${PROPERTY_LABEL} in ${location.county}`
                    : `${PROPERTY_LABEL} for Rent in ${location.county}`}
                </h2>
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
            ...
            {smartAlternatives.differentPropertyTypes.length > 0 && (
              <div className="mb-12">
                ...
                <p className="text-gray-600 mb-6">
                  {searchParams?.bedrooms
                    ? `Check out these ${searchParams.bedrooms}-bedroom properties (Offices, Shops, Warehouses, etc.) ${searchParams?.city ? `in ${searchParams.city}` : `in ${location.name}`}:`
            ...
            {smartAlternatives.sameCityDifferentBedrooms.length === 0 &&
             ...
             alternativeProperties.length > 0 && (
              <div className="mb-12">
                ...
                <p className="text-gray-600 mb-6">
                  Check out these properties for rent in {location.county}
                </p>
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {properties.length > 0 && properties.length < 3 && alternativeProperties.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              More {PROPERTY_LABEL} for Rent in {location.county}
            </h2>
            <p className="text-gray-600 mb-6">
              Explore more properties for rent in nearby areas
            </p>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
